...
<h2>Γύρος <?= $rn ?></h2>
<table>
    <thead><tr><th>Πίστα</th><th class="right">Γηπεδούχος</th><th class="tc">Σκορ</th><th>Φιλοξενούμενος</th></tr></thead>
    <tbody>
    <?php foreach ($matches as $m):
        $homeLabel = $labels[(int)$m['home_team_id']] ?? ('#' . (int)$m['home_team_id']); ?>
        <?php if ((int)$m['is_bye'] === 1): ?>
            <tr><td><?= (int)$m['board_no'] ?></td><td class="right"><?= h($homeLabel) ?></td><td class="tc" colspan="2">ΡΕΠΟ (<?= (int)$m['home_score'] ?>:<?= (int)$m['away_score'] ?>)</td></tr>
        <?php else:
            $awayLabel = $labels[(int)$m['away_team_id']] ?? ('#' . (int)$m['away_team_id']);
            $sc = $m['status'] === 'played' ? ((int)$m['home_score'] . ':' . (int)$m['away_score']) : '__ : __'; ?>
            <tr><td><?= (int)$m['board_no'] ?></td><td class="right"><?= h($homeLabel) ?></td><td class="tc"><?= h($sc) ?></td><td><?= h($awayLabel) ?></td></tr>
        <?php endif; ?>
    <?php endforeach; ?>
    </tbody>
</table>
