...
### Διαχωρισμός Ανδρών / Γυναικών (κατηγορίες)

Κατά τη δημιουργία διοργάνωσης, ανάλογα με το είδος του πρωταθλήματος:

- **Μικτό (Mixed)** → **ένα ενιαίο ταμπλό** (εισάγει ομάδες κατηγορίας `MIX`).
- **Διπλέτες / Τριπλέτες** → **δύο ξεχωριστά ταμπλό** που τρέχουν παράλληλα:
  **Άνδρες** (`M`) και **Γυναίκες** (`F`). Κάθε ταμπλό έχει δικές του ομάδες,
  γύρους, κληρώσεις, γήπεδα, ρεπό, αποτελέσματα, κατάταξη και knockout.
- Το **Interclub / Intercup** δεν υποστηρίζεται (εκτός αντικειμένου).

### Ρυθμίσεις διοργάνωσης (admin → ⚙ Ρυθμίσεις)

- **Πλήθος γηπέδων** — στην κλήρωση κάθε αγώνας παίρνει αριθμό γηπέδου κυκλικά
  (0 = χωρίς όριο, σειριακή αρίθμηση).
- **Προγραμματισμένοι γύροι** Ελβετικού — μπλοκάρεται νέα κλήρωση μόλις
  συμπληρωθούν.
- **Knockout**: `Χωρίς` / **TOP-8** / **TOP-16** (ανεξάρτητα ανά ταμπλό).
- **Κύπελλο Φιλίας** (θέσεις 17–32 της γενικής κατάταξης).
- Βαθμοί νίκης/ισοπαλίας/ήττας και σκορ ρεπό.

### 2η φάση — Knockout (νοκ-άουτ)

Μετά την ολοκλήρωση όλων των γύρων Ελβετικού:

- Ζευγάρωμα 1ου σταδίου από τη γενική κατάταξη: **1–16, 2–15, … 8–9** (TOP-16)
  ή **1–8, 2–7, 3–6, 4–5** (TOP-8).
- Εξέλιξη: Φάση των 16 → Προημιτελικά → Ημιτελικά → **Μικρός Τελικός**
  (ηττημένοι ημιτελικών) + **Μεγάλος Τελικός**.
- **Κύπελλο Φιλίας**: ίδια λογική single-elimination για τις θέσεις 17–32.
- Κάθε επόμενο στάδιο κληρώνεται μόνο όταν ολοκληρωθεί το προηγούμενο.

### Φύλλα αγώνα (εκτύπωση)

- Μετά από **κάθε κλήρωση** ανοίγει αυτόματα η εκτύπωση των φύλλων αγώνα.
- Διαθέσιμα και από κάθε γύρο (κουμπί «🖨 Φύλλα αγώνα») και από τη Γραμματεία.
- **3 φύλλα ανά σελίδα Α4** (portrait) — με πρωτάθλημα, κατηγορία, γύρο/στάδιο,
  γήπεδο, ομάδες και κενά πεδία σκορ. Ενδεικτικό αρχείο:
  `public/admin/tournament_match_sheets.php?id=N&round=R`.

### Γραμματεία αγώνων

Ξεχωριστή σελίδα (`public/admin/tournament_secretariat.php`) **μόνο για
καταχώρηση αποτελεσμάτων** ανά γύρο/στάδιο — χωρίς πρόσβαση στις ρυθμίσεις,
τις κληρώσεις ή τη διαγραφή. Οι ρυθμίσεις μένουν στη σελίδα ⚙ Ρυθμίσεις.

### Δημόσια ζωντανά αποτελέσματα (hpf.gr)

Δημόσια σελίδα **χωρίς login**: `public/results.php` (Bootstrap 5, αυτόματη
ανανέωση κάθε 60″). Δείχνει κατάταξη, γύρους, γήπεδα, ζευγάρια και σκορ όλων
των ταμπλό. Για το **hpf.gr** τη συνδέεις/ενσωματώνεις ως εξής:

- **Σύνδεσμος**: νέα σελίδα/μενού στο hpf.gr προς `https://<subdomain>/results.php`.
- **Ενσωμάτωση (iframe)** σε σελίδα του hpf.gr:
  ```html
  <iframe src="https://<subdomain>/results.php" style="width:100%;height:1200px;border:0"></iframe>
  ```

### Bootstrap 5

Το UI φορτώνει **self-hosted Bootstrap 5.3.3** από
`public/assets/vendor/bootstrap/` (μαζί με το υπάρχον `assets/style.css`).

### Εγκατάσταση / αναβάθμιση πινάκων

**Νέα εγκατάσταση** — περιλαμβάνονται ήδη στο `sql/schema.sql` (clean) και στο
`sql/migration_hostinger.sql` (integration). Εναλλακτικά μόνο η μηχανή αγώνων:

```bash
mysql -u root -p petanque < sql/tournaments.sql
```

**Αναβάθμιση υπάρχουσας εγκατάστασης** (αν είχες παλιά έκδοση των πινάκων
`app_tournament*` χωρίς τις νέες στήλες — κατηγορία, γήπεδα, knockout, φάσεις):

```bash
mysql -u <user> -p <db> < sql/upgrade_tournaments.sql
```

Ασφαλές να τρέξει πολλές φορές (MariaDB / Hostinger). Το ίδιο ALTER block
υπάρχει και μέσα στο `sql/migration_hostinger.sql`.

Η μηχανή γράφει **μόνο** στους `app_tournament*` πίνακες και διαβάζει ομάδες
από τον πίνακα `teams` (ή `app_teams` στο Hostinger integration, μέσω
`config tables.teams`). Καθαρή λογική στο `src/swiss.php` (Ελβετικό) και
`src/knockout.php` (νοκ-άουτ)· data layer στο `src/tournament.php`.
